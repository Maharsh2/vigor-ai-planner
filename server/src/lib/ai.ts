      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert fitness trainer and program designer. You must respond with valid JSON only. The array of days MUST be strictly named 'weeklySchedule' (do not use 'weekSchedule'). Do not include any markdown, reasoning, or additional text.",
        },
        {
          role: "user",
          content: prompt,
...
function formatPlanResponse(
  ...
  const plan: Omit<TrainingPlan, "id" | "userId" | "version" | "createdAt"> = {
    ...
    // The fallback logic: Catch 'weeklySchedule', 'weekSchedule', or default to an empty array
    weeklySchedule: (aiResponse.weeklySchedule || aiResponse.weekSchedule || []).map((day: any) => ({
      day: day.day || "Day",
